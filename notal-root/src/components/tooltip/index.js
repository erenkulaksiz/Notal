    ...
    >
        ...
        {show && <TooltipPortal parent={containerRef.current} className="pointer-events-none absolute bottom-[calc(100%+5px)]">
            <motion.div
                ...
                className="relative bg-neutral-100 dark:bg-neutral-900 whitespace-nowrap px-3 py-1 flex items-center justify-center rounded-xl text-sm shadow-xl z-50 top-0"
                onAnimationComplete={() => !visible && setShow(false)}
            >
                {content}
                <div className="w-4 -z-10 h-4 bg-neutral-100 dark:bg-neutral-900 absolute -bottom-1 rotate-45" />
            </motion.div>
        </TooltipPortal>}
    </div>)
}
