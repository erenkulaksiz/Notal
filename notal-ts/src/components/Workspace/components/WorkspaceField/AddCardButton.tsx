...
export function AddCardButton({ fieldId }: { fieldId: string }) {
  ...
  function addCard() {
    ...
    if (
      input.trim().length < LIMITS.MIN.WORKSPACE_CARD_TITLE_CHARACTER_LENGTH
    ) {
      return;
    }
    if (
      input.trim().length > LIMITS.MAX.WORKSPACE_CARD_TITLE_CHARACTER_LENGTH
    ) {
  ...
  return (
    <div className="flex flex-col relative w-full opacity-0 hover:opacity-100 transition-all ease-in-out pb-2">
      ...
      <Input
        ...
        onEnterPress={() =>
          input?.trim().length >=
            LIMITS.MIN.WORKSPACE_CARD_DESC_CHARACTER_LENGTH && addCard()
        }
        height="h-20"
