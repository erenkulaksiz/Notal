        ...
        <div className="relative hover:z-40 active:scale-[98%] hover:scale-[101%] hover:-translate-y-0.5 group transition-all ease-in-out">
            ...
            >
                <div className="flex flex-row justify-between group-hover:scale-[97%] transition-all ease-in-out z-20 pointer-events-none">
                    <div className="flex items-start justify-end flex-col text-white max-w-[calc(100%-60px)]">
                        ...
                        {!preview ? <Link href="/workspace/[id]" as={`/workspace/${workspace.id || "not-found"}`} passHref>
                            <a className="flex-col flex w-full">
                                ...
                                {workspace?.desc && <span className="text-sm drop-shadow-lg break-words">
                                    {workspace?.desc}
                                </span>}
                            </a>
                    ...
                    {!preview && <div className="flex flex-col justify-end pointer-events-auto drop-shadow">
                ...
                {(workspace?.thumbnail && workspace?.thumbnail?.type == "image") && <div className="absolute left-0 right-0 top-0 bottom-0 rounded-xl overflow-hidden z-0">
                    ...
                    {preview && <div className="cursor-pointer bg-gradient-to-t from-black/30 dark:from-black/90 dark:to-trasparent to-black/30 group-hover:opacity-0 opacity-100 transition-all ease-in-out absolute left-0 right-0 top-0 bottom-0 z-10" />}
                    <img
